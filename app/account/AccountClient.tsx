  ...
  return (
    <div className="max-w-2xl mx-auto p-4 md:p-8 space-y-6">
      <header className="space-y-2">
        <div className="font-mono text-[12px] tracking-[0.3em] text-muted">OPERATOR PROFILE</div>
      ...
      <section className="border border-border rounded-lg bg-surface/40 p-4">
        <div className="font-mono text-[12px] tracking-[0.2em] text-muted mb-3">
          NOTIFICATIONS
        </div>
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="max-w-sm">
            <div className="font-mono text-xs text-foreground">Alert digest email</div>
            <p className="font-mono text-[12px] text-muted mt-1 leading-relaxed">
          ...
          <button
            onClick={toggle}
            disabled={saving}
            className={`px-3 py-1.5 font-mono text-[12px] tracking-wider rounded border transition-colors disabled:opacity-50 ${
              optIn
        ...
        {status && (
          <p className="mt-3 font-mono text-[12px] text-muted-foreground">{status}</p>
        )}
      </section>

      <section className="border border-border rounded-lg bg-surface/40 p-4">
        <div className="font-mono text-[12px] tracking-[0.2em] text-muted mb-3">TIER LIMITS</div>
...
function Row({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <div className="flex items-baseline justify-between gap-4">
      <span className="font-mono text-[12px] tracking-[0.2em] text-muted">{label}</span>
